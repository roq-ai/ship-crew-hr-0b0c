...
import { getCrewMembers } from 'apiSdk/crew-members';
import { CrewMemberInterface } from 'interfaces/crew-member';
import { getLogs } from 'apiSdk/logs';
import { LogInterface } from 'interfaces/log';
import { getShips } from 'apiSdk/ships';
import { ShipInterface } from 'interfaces/ship';

import { CrewMemberListPage as ShowCaseList } from 'pages/crew-members';

function DashboardPage() {
  const { session } = useSession();

  const crewMemberStat = useSWR<PaginatedInterface<CrewMemberInterface>>(
    () => `/crew-members`,
    () => getCrewMembers({ limit: 1, offset: 0 }),
  );

  const logStat = useSWR<PaginatedInterface<LogInterface>>(
    () => `/logs`,
    () => getLogs({ limit: 1, offset: 0 }),
  );

  const shipStat = useSWR<PaginatedInterface<ShipInterface>>(
    () => `/ships`,
    () => getShips({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Crew Members',
      result: crewMemberStat?.data?.totalCount,
      isLoading: crewMemberStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Logs',
      result: logStat?.data?.totalCount,
      isLoading: logStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Ships',
      result: shipStat?.data?.totalCount,
      isLoading: shipStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`crew-members`} passHref legacyBehavior>
                  <Button
