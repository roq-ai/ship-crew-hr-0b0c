...
import { getShips, deleteShipById } from 'apiSdk/ships';
import { ShipInterface } from 'interfaces/ship';

type ColumnType = ColumnDef<ShipInterface, unknown>;
...
export function ShipListPage(props: ShipListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getShips({
        relations: ['company', 'crew_member.count', 'schedule.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ShipInterface>>(
    () => `/ships?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ShipInterface) => {
    if (hasAccess('ship', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/ships/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('company', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('crew_member', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'crew_member',
          header: 'Crew Member',
          accessorKey: 'crew_member',
          cell: ({ row: { original: record } }: any) => record?._count?.crew_member,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/ships/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('ship', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/ships/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('ship', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="ship" />
        </Flex>

        {hasAccess('ship', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/ships/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
