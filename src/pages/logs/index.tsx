...
type ColumnType = ColumnDef<LogInterface, unknown>;
...
export function LogListPage(props: LogListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<LogInterface>>(
    () => `/logs?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/logs/view/${record.id}`} passHref legacyBehavior>
                <Button
