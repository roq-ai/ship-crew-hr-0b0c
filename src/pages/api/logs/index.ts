...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getLogsPublic() {
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'log');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.log.count(countOptions as unknown),
      prisma.log.findMany({
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function getLogs() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.log
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'log'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
  ...
  async function createLog() {
    await logValidationSchema.validate(req.body);
    const body = { ...req.body };
